...
static ssize_t show_gpu_voltage_control(struct cpufreq_policy *policy, char *buf)
{
	int i = 0;
	char *out = buf;
	struct clk *gpu_clk_threed = tegra_get_clock_by_name("3d");

   rcu_read_lock();

	/* find how many actual entries there are */
	i = gpu_clk_threed->dvfs->num_freqs;

	for(i--; i >=0; i--) {
		out += sprintf(out, "%lumhz: %i mV\n",
				gpu_clk_threed->dvfs->freqs[i]/1000000,
				gpu_clk_threed->dvfs->millivolts[i]);
	}
...
static ssize_t store_gpu_voltage_control(struct cpufreq_policy *policy, const char *buf, size_t count)
{
	int i = 0;
	unsigned long volt_cur;
	int ret;
   ...
   rcu_read_lock();
	...
	for(i--; i >= 0; i--) {
		...
		if(gpu_clk_threed->dvfs->freqs[i]/1000000 != 0) {
			ret = sscanf(buf, "%lu", &volt_cur);
			if (ret != 1)
				return -EINVAL;

			/* TODO: need some robustness checks */
			pr_info("freq [%i]: new mv: %lu old mv: %lu\n", gpu_clk_threed->dvfs->freqs[i]/1000000, volt_cur, gpu_clk_threed->dvfs->millivolts[i]);
			gpu_clk_threed->dvfs->millivolts[i] = volt_cur;
...
cpufreq_freq_attr_rw(gpu_voltage_control);
#endif
