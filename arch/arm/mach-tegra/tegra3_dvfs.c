...
int core_millivolts[] = { 950,   1000,   1050,   1100,   1150,    1200,    1250,   1300, 1350,   1375,   1412,   1437,   1462 };
...
static struct dvfs core_dvfs_table[] = {
   /* soc_id2 == TF201 && TF700T */
	...
	CORE_DVFS("cpu_lp",     2, 1, KHZ, 204000,   295000,   370000,   428000,   475000,      513000,      579000,   620000,   620000,   760000,   760000,   760000,   760000),
...
void static inline hundsbuah_set_dvfs_table_for_each_cpu_id_and_proc_id_individually(void)
{
	...
	if(cpu_speedo_id == 5) /* TF700T */
	{
		if(cpu_process_id == 3)
		{
			/* TODO: remove the 1900MHz entry from processor_id 3 because this processor hw revision/model cant handle 1900MHz ? */
		...
		{
			int millivolts_temp_array[] = {                                       750, 762, 775, 787, 800, 825, 837, 850,  875,  900,  925,  975,  987, 1000, 1025, 1050, 1062, 1075, 1100, 1112, 1125, 1150, 1200, 1212, 1275, 1350, 1387 };
			struct dvfs cpu_dvfs_table_temp[]   = { CPU_DVFS("cpu_g",  5, 4, MHZ,   1,   1,   1,   1, 550, 550, 550, 770,  770,  940,  940, 1160, 1160, 1240, 1280, 1360, 1360, 1390, 1470, 1470, 1500, 1520, 1590, 1700, 1750, 1800, 1850 ), };
			struct dvfs cpu_0_dvfs_table_temp[] = { CPU_DVFS("cpu_0",  5, 4, MHZ, 475, 620, 620, 620, 760, 760, 760, 910, 1000, 1000, 1150, 1150, 1300, 1300, 1400, 1400, 1500, 1500, 1500, 1600, 1600, 1600, 1600, 1700, 1750, 1800, 1850 ), };
			hundsbuah_fill_arrays(millivolts_temp_array, ARRAY_SIZE(millivolts_temp_array));
			memcpy((void *)cpu_dvfs_table,   (void *)cpu_dvfs_table_temp,   sizeof(cpu_dvfs_table_temp));
			memcpy((void *)cpu_0_dvfs_table, (void *)cpu_0_dvfs_table_temp, sizeof(cpu_0_dvfs_table_temp));
		}
	...
	else if(cpu_speedo_id == 3) /* TF201 fallback */
	{
		if(cpu_process_id == 1)
		{
		    int millivolts_temp_array[] = {                                       750, 800, 850, 900, 975, 1000, 1025, 1050, 1075, 1100, 1125, 1150, 1175, 1237, 1275, 1387 };
		    struct dvfs cpu_dvfs_table_temp[]   = { CPU_DVFS("cpu_g",  3, 1, MHZ,   1, 480, 650, 780, 990, 1040, 1100, 1200, 1250, 1300, 1330, 1400, 1500, 1600, 1700, 1800 ), };
		    hundsbuah_fill_arrays(millivolts_temp_array, ARRAY_SIZE(millivolts_temp_array));
			memcpy((void *)cpu_dvfs_table,   (void *)cpu_dvfs_table_temp,   sizeof(cpu_dvfs_table_temp));
		}
		else if(cpu_process_id == 2)
		{
			...
			struct dvfs cpu_dvfs_table_temp[]   = { CPU_DVFS("cpu_g",  3, 2, MHZ,   1, 520, 700, 860, 1050, 1200, 1280, 1300, 1350, 1400, 1500, 1600, 1700, 1800), };
			hundsbuah_fill_arrays(millivolts_temp_array, ARRAY_SIZE(millivolts_temp_array));
			memcpy((void *)cpu_dvfs_table,   (void *)cpu_dvfs_table_temp,   sizeof(cpu_dvfs_table_temp));
		}
		else if(cpu_process_id == 3)
		{
			...
			struct dvfs cpu_dvfs_table_temp[]   = { CPU_DVFS("cpu_g",  3, 3, MHZ,   1, 550, 770, 910, 1150, 1230, 1280, 1300, 1350, 1400, 1500, 1600, 1700, 1800), };
			hundsbuah_fill_arrays(millivolts_temp_array, ARRAY_SIZE(millivolts_temp_array));
			memcpy((void *)cpu_dvfs_table,   (void *)cpu_dvfs_table_temp,   sizeof(cpu_dvfs_table_temp));
		}
		else /* fallback */
...
void __init tegra_soc_init_dvfs(void)
{
	...
	/* __START: Fix reboot to recovery bug */
	hundsbuah_set_dvfs_table_for_each_cpu_id_and_proc_id_individually();
	/* __END: Fix reboot to recovery bug */
